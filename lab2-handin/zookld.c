...
pid_t launch_svc(CONF *conf, const char *name)
{
    ...
    if (NCONF_get_number_e(conf, name, "gid", &gid))
    {
        ...
        int fail = setresgid(gid, gid, gid);
        if(fail)    err(1,"setresgid failed\n");
        warnx("setgid %ld", gid);
    }
    ...
    if ((groups = NCONF_get_string(conf, name, "extra_gids")))
    {
        ...
        int fail = setgroups(ngids, gids);
        if(fail)    err(1,"setgroups failed\n");
        for (i = 0; i < ngids; i++)
    ...
    if ((dir = NCONF_get_string(conf, name, "dir")))
    {
        ...
        /*Ex 2*/
        int fail = chdir(dir);
        if(fail)    err(1,"chroot failed\n");
        fail = chroot(dir);
        if(fail)    err(1,"chdir failed\n");
    }

    if (NCONF_get_number_e(conf, name, "uid", &uid))
    {
        /* change real, effective, and saved uid to uid */
        int fail = setresuid(uid, uid, uid);
        if(fail)    err(1,"setresuid failed\n");
        warnx("setuid %ld", uid);
    }
