...
const char *http_request_line(int fd, char *reqpath, size_t reqpath_len, char *env, size_t *env_len, size_t max_env_len)
{
    ...
    int ret_val = http_read_line(fd, buf,sizeof(buf));
    if ( ret_val == -1)
        return "Socket IO error";
    if (ret_val == -2)
        return "The HTTP request size is too long";
    ...
    *sp2 = '\0';
    ...
    if (strlen(env)>12+strlen(reqpath))
        return "The HTTP request size is too long";
    envp += sprintf(envp, "REQUEST_URI=%s", reqpath) + 1;
    if (strlen(env)+22>max_env_len)
        return "The HTTP request size is too long";
...
const char *http_request_headers(int fd)
{
    ...
    for (;;)
    {
        ...
        if ( ret_val == -1)
            return "Socket IO error";
        if (ret_val == -2)
            return "The HTTP request size exceeds the buffer size, please send a smaller request";
...
void http_serve(int fd, const char *name)
{
    ...
    if (strlen(name)>sizeof(pn)-strlen(pn))
        http_err(fd,414,"Request-URI too long");
    strcat(pn,name);
    split_path(pn);
